...
function addRow() {
    ...
    newRow.innerHTML = `
        ...
        <td><input type="number" name="TANDSDETAILS[${rowNumber}].EMS_BUSFARE" oninput="calculateNextTotal(this, ${rowNumber})" class="form-control form-control-sm" ></td>
        <td><input type="number" name="TANDSDETAILS[${rowNumber}].EMS_ACCOMMODATION" oninput="calculateNextTotal(this, ${rowNumber})" class="form-control form-control-sm" ></td>
        <td><input type="number" name="TANDSDETAILS[${rowNumber}].EMS_LUNCH" oninput="calculateNextTotal(this, ${rowNumber})" class="form-control form-control-sm" ></td>
        <td><input type="number" name="TANDSDETAILS[${rowNumber}].EMS_DINNER" oninput="calculateNextTotal(this, ${rowNumber})" class="form-control form-control-sm" ></td>
        <td><input type="number" name="TANDSDETAILS[${rowNumber}].EMS_TOTAL" id="TANDSDETAILS[${rowNumber}].EMS_TOTAL" readonly class="form-control form-control-sm"></td>
        <td>
            <div class="d-flex">
...
function deleteRow(button) {
    ...
    // Get the EMS_TOTAL value from the row to be deleted
...
function calculateTotal(input) {
    // Get the closest table to the input element
    var parentTable = input.closest('table');

    // Get all input fields in the same row excluding the EMS_TOTAL, EMS_PLACE, EMS_ARRIVAL, EMS_DEPARTURE, and EMS_DATE fields
    var inputs = parentTable.querySelectorAll('input[name^="TANDSDETAILS"]:not([name$=".EMS_TOTAL"]):not([name$=".EMS_PLACE"]):not([name$=".EMS_ARRIVAL"]):not([name$=".EMS_DEPARTURE"]):not([name$=".EMS_DATE"])');

    // Initialize total sum
    var total = 0;

    // Loop through all inputs and sum their values
    inputs.forEach(function (inputField) {
        total += parseFloat(inputField.value) || 0; // Use 0 if the value is not a number
    });

    // Get the EMS_TOTAL field for this row
    var totalInput = parentTable.querySelector('input[name$=".EMS_TOTAL"]');

    // Debug: Check if the EMS_TOTAL input field is found
    if (totalInput) {
        // Update the EMS_TOTAL field with the calculated total
        totalInput.value = total.toFixed(2);
    } else {
        console.error('EMS_TOTAL input not found in the current row');
    }

    // Recalculate the grand total after row total is updated
    updateGrandTotal();
}

function updateGrandTotal() {
    // Select all input fields for EMS_TOTAL across all rows
    const inputFields = Array.from(document.querySelectorAll('[name^="TANDSDETAILS"][name$=".EMS_TOTAL"]'));

    // Get references to the total spans
    const totalSpan = document.getElementById('totala');
    const totalSpanB = document.getElementById('total');
    const grandTotalSpan = document.getElementById('totalF');

    if (!totalSpan || !totalSpanB || !grandTotalSpan) {
        console.error('One or more total spans are missing');
        return;
    }

    // Calculate the total of all EMS_TOTAL values
    let totalValueA = inputFields.reduce((sum, input) => sum + (parseFloat(input.value) || 0), 0);

    // Update the totalSpan with the sum of EMS_TOTAL values
    totalSpan.textContent = `$${totalValueA.toFixed(2)}`;

    // Add the total from totalSpanB to the grand total
    let grandTotalText = parseFloat(totalSpanB.textContent.replace(/[^0-9\.]/g, '')) + totalValueA;
    grandTotalSpan.textContent = `$${grandTotalText.toFixed(2)}`;
}
